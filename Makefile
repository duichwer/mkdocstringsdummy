...
check-safety:  ## Check for vulnerabilities in dependencies using safety.
	poetry run pip freeze 2>/dev/null | \
		grep -v mkdocstringsdummy | \
		poetry run safety check -i 36541 --stdin --full-report 2>/dev/null
	# Ignore urllib3 http vulnerability (ID: 36541)
...
.PHONY: publish-docs
publish-docs: docs ## Generate mkdocs
	mv site public
...
.PHONY: lint
lint: lint-isort lint-black  ## Run linting tools on the code.
...
test: clean-tests  ## Run the tests using pytest.
	...
	-poetry run pytest --cov==mkdocstringsdummy tests/
